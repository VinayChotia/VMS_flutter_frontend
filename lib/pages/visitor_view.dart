...
class _VisitorViewPageState extends State<VisitorViewPage> {
  ...
  Future<void> _initializeData() async {
    ...
    try {
      ...
      } else if (widget.visitorId != null && widget.fetchData) {
        // Fetch data by ID
        await _fetchVisitorData(widget.visitorId!);
      } else {
        setState(() => _isLoading = false);
      }
    } catch (e) {
  ...
  Future<void> _fetchVisitorData(int visitorId) async {
    try {
      final response = await ApiService.getVisitor(visitorId);
      setState(() {
        _visitorData = response;
  ...
  Future<void> _refreshData() async {
    if (widget.visitorId != null) {
      await _fetchVisitorData(widget.visitorId!);
    } else if (_visitorData.containsKey('id')) {
      await _fetchVisitorData(_visitorData['id']);
    }
  }
